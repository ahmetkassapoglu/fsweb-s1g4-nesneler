...
function MenuElemaniOlustur(i,f,k){
...
console.log(MenuElemaniOlustur("çorba",11,"sulu yemek"))
...
function DegerledirmeEkle(d,i,p,g){
 let son = {
	isim : i,
	puan: p,
	geribildirim: g
...
function AnahtardanDegerlendirmeAl(deger,sira) {
	return `${deger[sira].isim} isimli kişi ${deger[sira].puan} puan verdi ve şunları yazdı: ${deger[sira].geribildirim}`
...
function SonDegerlendirmeyiAl(d) {
	return `${d[d.length-1].isim} isimli kişi ${d[d.length-1].puan} puan verdi ve şunları yazdı: ${d[d.length-1].geribildirim}`
} 
...
function PuanaGoreDegerlendirmeAl(d,p) {
	let sonuc =[]
    for (let i=0; i< d.length-1 ; i++ ){
		if ( p <= d[i].puan && d[i].puan < p+1  ){
			sonuc.push(d[i])
		}
	}
	return sonuc
...
function UzunDegerlendirmeleriAl(d) {
    let sonuc =[]
    for (let i=0; i< d.length-1 ; i++ ){
		if ( d[i].geribildirim.length >15  ){
			sonuc.push(d[i])
		}
	}
	return sonuc
